...
def three_component_single_power_law_primary_mass(
    mass, alpha, mmin, mmax, lam_0, lam_1, mpp_1, sigpp_1, mpp_2, sigpp_2, gaussian_mass_maximum
    ):
    """
    A three-component single power law mass model.
    ...
    alpha: float
        The power-law index(:math:`\alpha`).
    mmin: float
...
class MultiPeakPowerLawSmoothedMassDistribution(BaseSmoothedMassDistribution):
    ...
    primary_model = three_component_single_power_law_primary_mass
...
class m2gap_MultiPeakPowerLawSmoothedMassDistribution(MultiPeakPowerLawSmoothedMassDistribution):
    ...
    def __call__(self, dataset, *args, **kwargs):
        beta = kwargs.pop("beta")
        A = kwargs.pop("A")
        eta_low = kwargs.pop("eta_low")
        eta_high = kwargs.pop("eta_high")
        mmin_1 = kwargs.pop("mlow_1", self.mmin)
        mmin_2 = kwargs.pop("mlow_2", self.mmin)
        delta_m_1 = kwargs.pop("delta_m_1", 0)
        delta_m_2 = kwargs.pop("delta_m_2", 0)
        ...
        p_m1 = self.p_m1(dataset, mmin=mmin_1, delta_m=delta_m_1, **kwargs, **self.kwargs)
        p_q = self.p_q(dataset, beta=beta, mmin=mmin_2, delta_m=delta_m_2, gap_low=gap_low, gap_width=gap_width, A=A, eta_low=eta_low, eta_high=eta_high)
        prob = p_m1 * p_q
        return prob

    def p_q(self, dataset, beta, mmin, delta_m, gap_low, gap_width, A, eta_low, eta_high):
        p_q = powerlaw(dataset['mass_ratio'], beta, 1, mmin / dataset['mass_1'])
        p_m2_notch = 1 - A / ((1+(gap_low/(dataset['mass_ratio']*dataset['mass_1']))**eta_low)*(1+((dataset['mass_ratio']*dataset['mass_1'])/(gap_low+gap_width))**eta_high))
        p_q *= p_m2_notch
        p_q *= self.smoothing(
        ...
        try:
            if self.cache:
                p_q /= self.norm_p_q(beta=beta, mmin=mmin, delta_m=delta_m, gap_low=gap_low, gap_width=gap_width, A=A, eta_low=eta_low, eta_high=eta_high)
            else:
    ...
    def norm_p_q(self, beta, mmin, delta_m, gap_low, gap_width, A, eta_low, eta_high):
        """Calculate the mass ratio normalisation by linear interpolation"""
        p_q = powerlaw(self.qs_grid, beta, 1, mmin / self.m1s_grid)
        p_m2_notch = 1 - A / ((1+(gap_low/(self.m1s_grid * self.qs_grid))**eta_low)*(1+((self.m1s_grid * self.qs_grid)/(gap_low+gap_width))**eta_high))
        p_q *= p_m2_notch
        p_q *= self.smoothing(
    ...
    def variable_names(self):
        ...
        vars += ["beta", "delta_m_1", "delta_m_2", "mlow_1", "mlow_2", "gap_low", "gap_width", "A", "eta_low", "eta_high"]
        vars.remove("mmin")
        vars = set(vars).difference(self.kwargs.keys())
        return vars
...
priors = dict(
    mass = dict(
        ...
        A = bilby.prior.Uniform(0, 1),
        eta_low = bilby.prior.Uniform(0, 50),
        eta_high = bilby.prior.Uniform(0, 50),
        H0 = 67.90,
        Om0 = 0.3065,
    )
)