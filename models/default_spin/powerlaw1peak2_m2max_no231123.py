...
class m2max_MultiPeakPowerLawSmoothedMassDistribution(MultiPeakPowerLawSmoothedMassDistribution):
    ...
    def __call__(self, dataset, *args, **kwargs):
        ...
        p_m1 = self.p_m1(dataset, mmin=mmin_1, delta_m=delta_m_1, **kwargs, **self.kwargs)
        p_q = self.p_q(dataset, beta=beta, mmin=mmin_2, delta_m=delta_m_2, m2max=m2max)
        prob = p_m1 * p_q
        return prob

    def p_q(self, dataset, beta, mmin, delta_m, m2max):
        p_q = 0.0

        p_q += (dataset['mass_1'] <= m2max) * powerlaw(
            dataset['mass_ratio'], beta, 1, mmin / dataset['mass_1'],
        )
        p_q += (dataset['mass_1'] > m2max) * powerlaw(
            dataset['mass_ratio'], beta, m2max / dataset['mass_1'], mmin / dataset['mass_1'],
        )

        p_q *= self.smoothing(
            dataset["mass_1"] * dataset["mass_ratio"],
            mmin=mmin,
            mmax=xp.where(dataset["mass_1"]<=m2max,dataset["mass_1"],m2max),
            delta_m=delta_m,
        )
        try:
            if self.cache:
                p_q /= self.norm_p_q(beta=beta, mmin=mmin, delta_m=delta_m, m2max=m2max)
            else:
    ...
    def norm_p_q(self, beta, mmin, delta_m, m2max):
        """Calculate the mass ratio normalisation by linear interpolation"""
        p_q = 0.0
        p_q += (self.m1s_grid <= m2max) * powerlaw(
            self.qs_grid, beta, 1, mmin / self.m1s_grid,
        )
        p_q += (self.m1s_grid > m2max) * powerlaw(
            self.qs_grid, beta, m2max / self.m1s_grid, mmin / self.m1s_grid,
        )

        p_q *= self.smoothing(
            self.m1s_grid * self.qs_grid, mmin=mmin, mmax=xp.where(self.m1s_grid<=m2max,self.m1s_grid,m2max), delta_m=delta_m
        )
    ...
    def variable_names(self):
        ...
        vars += ["beta", "delta_m_1", "delta_m_2", "mlow_1", "mlow_2", "m2max"]
        vars.remove("mmin")
        vars = set(vars).difference(self.kwargs.keys())
        return vars
...
models = dict(
    mass = m2max_MultiPeakPowerLawSmoothedMassDistribution,
)
...
priors = dict(
    mass = dict(
        ...
        m2max = bilby.prior.Uniform(20, 150),
        H0 = 67.90,
        Om0 = 0.3065,
    )
)