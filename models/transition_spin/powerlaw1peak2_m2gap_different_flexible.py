...
class Transition_chi_eff:
    @property
    def variable_names(self):
        vars = ["m_t",  "chi_eff_min_high", "chi_eff_max_high", "log_sigma_chi_eff_low", "mu_chi_eff_low"]
        return vars

    def __call__(self, dataset, *args, **kwargs):
        m_t = kwargs['m_t']
        chi_eff_min_high = kwargs['chi_eff_min_high']
        chi_eff_max_high = kwargs['chi_eff_max_high']
        ...
        p_chi += (dataset['mass_1']>=m_t) * self.p_Uniform_chi_eff(dataset['chi_eff'], chi_eff_min_high, chi_eff_max_high)

        return p_chi
    
    def p_Uniform_chi_eff(self, chi_eff, chi_eff_min, chi_eff_max):

        p = ((chi_eff >= chi_eff_min) * (chi_eff <= chi_eff_max)) * 1/(chi_eff_max-chi_eff_min)
...
priors = dict(
    ...
    chi_eff = dict(
        ...
        chi_eff_min_high = bilby.prior.Uniform(-1, 1),
        chi_eff_max_high = bilby.prior.Uniform(-1, 1),
        chi_eff_width = bilby.prior.Constraint(0, 10),
